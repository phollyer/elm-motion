...
| --- | --- | --- |
| `retarget` | [Sub](../engines/sub.md), [WAAPI](../engines/waapi.md), [Transition](../engines/transition.md), [Keyframe](../engines/keyframes.md) | Change target while an animation is running, reusing its current timing. Sub and WAAPI interpolate smoothly from the live value; Transition and Keyframe snap to the new end value (no runtime snapshot to interpolate from). |
