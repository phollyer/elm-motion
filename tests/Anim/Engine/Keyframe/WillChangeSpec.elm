module Anim.Engine.Keyframe.WillChangeSpec exposing (suite)

{-| Verifies the Keyframe engine emits a `will-change` declaration
covering the properties currently animating. The Keyframe engine
renders all transforms via the composite `transform:` property, so
every transform-family property collapses to a single `transform`
entry.

The clearing-on-complete branch piggybacks on the same `AnimGroup.isComplete`
gate used by the rest of the engine; infinite-loop animations never
reach `isComplete` and so retain `will-change` indefinitely.
