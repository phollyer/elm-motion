...
- Per-axis `Unit` overrides on `Anim.Property.Translate` (`lengthX`, `lengthY`, `lengthZ`), `Anim.Property.Size` (`lengthWidth`, `lengthHeight`), and `Anim.Property.PerspectiveOrigin` (`lengthX`, `lengthY`). Each axis falls back to the property-level `length` (then engine default, then `Px`), so mixed-unit animations like `translate3d(50%, 100px, 0px)` are first-class. The WAAPI wire format now emits per-axis `unitX`/`unitY`/`unitZ` (translate), `unitWidth`/`unitHeight` (size), and `unitX`/`unitY` (perspectiveOrigin); the JS companion renders each axis independently in keyframes and `parseTransformString` recovers per-axis units from inline styles.
