...
- WAAPI `size` payload now carries a `unit` field reflecting `Size.length`, and the JS companion renders `width`/`height` keyframes with the configured unit instead of hard-coded `px`. Existing call sites that don't set `Size.length` continue to render in `px`.
