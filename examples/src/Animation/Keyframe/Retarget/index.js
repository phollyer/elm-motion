...
var $author$project$Anim$Internal$Builder$setBaselinesFromProcessedEnds = F3(
	function (animGroupName, props, _v0) {
		...
		var existing = A2(
			$elm$core$Maybe$withDefault,
			$author$project$Anim$Internal$Builder$PropertyBaselines$empty,
			A2($author$project$Anim$Internal$Engine$Shared$AnimGroups$get, animGroupName, state.L));
		...
		return _Utils_update(
			data,
			{
				s: _Utils_update(
					state,
					{
						L: A3($author$project$Anim$Internal$Engine$Shared$AnimGroups$insert, animGroupName, merged, state.L)
					})
			});
	});
			...
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							S: A2($author$project$Anim$Engine$Keyframe$reset, $author$project$Animation$Keyframe$Retarget$Main$animGroup, model.S)
						}),
					$elm$core$Platform$Cmd$none);
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Animation$Keyframe$Retarget$Main$animationArea(model.S)
			]));
};
